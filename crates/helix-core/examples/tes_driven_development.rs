...
mod tests {
    ...
    fn test_calculator_add_comprehensive() {
        ...
        assertions += 1;
        assert_eq!(calc.add(0, 0).unwrap(), 0);
        assertions += 1;
        assert_eq!(calc.add(-5, 5).unwrap(), 0);
        assertions += 1;
        assert_eq!(calc.add(-10, -20).unwrap(), -30);
        assertions += 1;

        // Edge cases
        assert_eq!(calc.add(i32::MAX, 0).unwrap(), i32::MAX);
        assertions += 1;
        assert_eq!(calc.add(i32::MIN, 0).unwrap(), i32::MIN);
        assertions += 1;

        // Error conditions
        assert!(calc.add(i32::MAX, 1).is_err());
        assertions += 1;
        assert!(calc.add(i32::MIN, -1).is_err());
        assertions += 1;
        assert_eq!(calc.add(i32::MAX, 1).unwrap_err(), "Overflow occurred");
        assertions += 1;

        // Behavior verification
        assert_eq!(calc.history().len(), 6);
        assertions += 1;
        assert!(calc.history()[0].contains("add(2, 3) = 5"));
        assertions += 1;

        let duration = start.elapsed();
        println!("Test: test_calculator_add_comprehensive");
        println!(
            "Assertions: {} (density: {:.1})",
            assertions,
            assertions as f64 / 3.0
        );
    ...
    fn test_calculator_divide_comprehensive() {
        ...
        assertions += 1;
        assert_eq!(calc.divide(15, 3).unwrap(), 5);
        assertions += 1;
        assert_eq!(calc.divide(-20, 4).unwrap(), -5);
        assertions += 1;
        assert_eq!(calc.divide(7, 2).unwrap(), 3);
        assertions += 1; // Integer division

        // Edge cases
        assert_eq!(calc.divide(0, 5).unwrap(), 0);
        assertions += 1;
        assert_eq!(calc.divide(1, 1).unwrap(), 1);
        assertions += 1;
        assert_eq!(calc.divide(-1, -1).unwrap(), 1);
        assertions += 1;
        ...
        assertions += 1;
        assert_eq!(calc.divide(5, 0).unwrap_err(), "Division by zero");
        assertions += 1;

        // History verification
        assert_eq!(calc.history().len(), 7);
        assertions += 1;
        assert!(calc.history().iter().all(|h| h.contains("divide")));
        assertions += 1;
    ...
    fn test_calculator_history_management() {
        ...
        assertions += 1;
        assert!(calc.history()[0].starts_with("add"));
        assertions += 1;
        assert!(calc.history()[1].starts_with("divide"));
        assertions += 1;
