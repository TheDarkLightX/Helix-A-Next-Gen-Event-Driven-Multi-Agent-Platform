...
use super::{evolution::Individual, Mutation, MutationType};
...
fn display_generation_results(data: &GenerationData) {
    ...
    let fitness_emoji = if data.avg_fitness >= 0.8 {
        "🟢"
    } else if data.avg_fitness >= 0.6 {
        "🟡"
    } else {
        "🔴"
    };
    ...
    println!(
        "   💀 Kill Rate: {}/{} ({:.1}%)",
        data.mutations_killed, data.total_mutations, kill_rate
    );
...
async fn demonstrate_tes_evolution() {
    ...
    let tes_evolution = vec![
        ...
        (
            ...
            0.52,
            "D",
            "Slight improvement through selection",
        ),
        (
            "Generation 3",
            0.64,
            "C",
            "Crossover improves mutation detection",
        ),
        (
            "Generation 4",
            0.73,
            "B",
            "Mutation adds effective diversity",
        ),
...
pub fn demonstrate_evolutionary_operators() {
    ...
    let individual1 = create_sample_individual(
        1,
        vec![
            (
                "profile.rs",
    ...
    let individual2 = create_sample_individual(
        2,
        vec![
            (
                "profile.rs",
    ...
    let child = create_sample_individual(
        3,
        vec![
            (
                "profile.rs",
    ...
    let mutated = create_sample_individual(
        4,
        vec![
            (
                "profile.rs",
...
fn create_sample_individual(
    ...
    let mutation_objects: Vec<Mutation> = mutations
        .into_iter()
        .map(|(file, line, mut_type, orig, mutated)| Mutation {
            id: Uuid::new_v4(),
...
mod tests {
    ...
    fn test_sample_individual_creation() {
        let individual = create_sample_individual(
            1,
            vec![("test.rs", 10, MutationType::BooleanLiteral, "true", "false")],
            0.5,
        );
