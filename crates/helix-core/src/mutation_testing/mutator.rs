...
impl Mutator {
    ...
    pub fn generate_file_mutations(
        &self,
        file_path: &PathBuf,
    ) -> Result<Vec<Mutation>, HelixError> {
        let code = fs::read_to_string(file_path).map_err(HelixError::IoError)?;

        self.operator.mutate(&code, file_path.as_path())
    }

    /// Apply a specific mutation to a file's content
    pub fn apply_mutation_to_code(
        &self,
        code: &str,
        mutation: &Mutation,
