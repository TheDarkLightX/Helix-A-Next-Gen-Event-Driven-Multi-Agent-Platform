...
use super::evaluator::{DefaultFitnessEvaluator, MutationEvaluator};
use super::mutator::Mutator;
use super::{FitnessEvaluator, Mutation, MutationConfig, MutationResult, MutationStrategy};
...
impl EvolutionaryMutationTester {
    ...
    async fn evaluate_population(
        ...
        for individual in population.iter_mut() {
            ...
            for mutation in &individual.mutations {
                // Read original file
                let original_code =
                    fs::read_to_string(&mutation.file_path).map_err(HelixError::IoError)?;
                ...
                let result = self
                    .evaluator
                    .evaluate_mutation(mutation, &mutated_code)
                    .await?;
    ...
    ) -> Result<Vec<Individual>, HelixError> {
